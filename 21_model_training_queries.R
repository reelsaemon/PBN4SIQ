...
training_size_values <- c(0.99)
splitting_method_values <- c("chronological")
n_samples_values <- c(10000)
n_bins_values <- c(3)
gen_bin_names_values <- c(TRUE)
other_additional_attributes_values <- c(FALSE)
time_features_values <- c(TRUE)
binning_method_values <- c("plain")
use_quantile_binning_values <- c(TRUE)
dataset_values <- c(
  "helpdesk"
)
random_seed_values <- c(27626)
n_case_duration_classes_values <- c(10)
outcome_query_values <- c(TRUE)
...
parameter_combinations <- as.data.table(expand.grid(list(k_values, 
                                                         ...
                                                         dataset_values,
                                                         random_seed_values,
                                                         n_case_duration_classes_values,
                                                         outcome_query_values)))
...
colnames(parameter_combinations) <- c("k", "repl_unident", "training_size", "splitting_method", 
                                      ...
                                      "dataset", "random_seed", "n_case_duration_classes", "outcome_query")
...
median_case_duration <- round(case_durations[, median(duration)])
event_log[, case_duration_binned:=cut(case_duration, quantile(case_duration, probs=seq(0, 1, 1/n_case_duration_classes)), include.lowest=TRUE)]
...
# generating outcome activities -------------------------------------------

# add duration outcome to finished variable for duration querying
activity_nodes <- colnames(train_wide)[grepl("^at_time_", colnames(train_wide))]
train_wide <- left_join(train_wide, case_durations)
train_wide <- left_join(train_wide, case_durations_binned)

if(outcome_query==TRUE) {
    
  if(is.null(n_case_duration_classes)) {
    
    # for binary split at median case duration
    train_wide[, (activity_nodes):=lapply(.SD, 
                                          function(x) if_else(duration <= median_case_duration, 
                                                              if_else(x=="finished", paste0("finished_le_", median_case_duration, "_hours"), x), 
                                                              if_else(x=="finished", paste0("finished_g_", median_case_duration, "_hours"), x)
                                          )
    ), 
    .SDcols=activity_nodes, 
    by=get(id_colname)]
    
    # update factor levels
    updated_levels <- setdiff(train_wide[, levels(get(activity_nodes[length(activity_nodes)]))], "finished")
    
    train_wide[, (activity_nodes):=lapply(.SD, function(x) factor(x, levels=updated_levels)), .SDcols=activity_nodes]
  } else if(!is.null(n_case_duration_classes)) {
    
    # for split along case duration classes
    train_wide[, (activity_nodes):=lapply(.SD, function(x) if_else(x=="finished", 
                                                                   if_else(!is.na(duration_binned), paste0(x, "_class_", duration_binned), NA), 
                                                                   x)), .SDcols=activity_nodes, by=get(id_colname)]
